...
1. **Hotel**: Represents the static information about each property, including its name, location, rating, images, and metadata used for search and display.

2. **Room_Type**: Defines the different categories of rooms within a hotel, capturing details such as capacity, amenities, and unique identifiers used by other services.

3. **Inventory**: Stores per-room-type, per-date availability, acting as the authoritative source of truth for booking operations and powering hotel search queries.

4. **Price**: Manages pricing information, including base rate snapshots for display as well as rules and promotions for dynamic or personalized pricing.

5. **Reservation**: Represents confirmed customer bookings, including the selected hotel, room types, dates, amounts charged, and customer details.
