...
Join the availability data with hotel metadata, group room types by hotel, and for each hotel call the `PricingService` to fetch rates. Compute the average per-night price across the stay for each room type, select the lowest among them, and assemble an `AvailableHotelSummary` with the hotel’s static details and the chosen price.
