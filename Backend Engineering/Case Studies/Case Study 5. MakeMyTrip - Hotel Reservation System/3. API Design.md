...
<span style="color:purple;font-weight:bold">1. GET /search/hotels?destination=...&check-in-date=...&check-out-date=...</span>
...
public List<AvailableHotelSummary> search(String destination, 
        ...
        /*
         * write step-by-step algorithm here...
         */
...
Step 1: Query `HotelService` for hotel metadata

```
List<Hotel> hotels = hotelServiceClient.getHotelsByDestination(destination);
```

Step 2: Query `InventoryService` to get list of room types which have rooms available on every date in the `[checkInDate, checkOutDate)` date range.
...
List<AvailableRoomType> availableRoomTypes = inventoryServiceClient
            .getAvailableRoomTypes(new InventoryQueryRequest(hotelIds, 
...
[
    ...
    },
    {
      ...
      "name": "Westin Resort",
      "locationId": "GOA",
      "thumbnailUrl": "https://cdn.../102/thumb.jpg",
      "rating": 4.7,
      "starRating": 5,
...
Will be same as that of the `GET /search/hotels` of the search service.
