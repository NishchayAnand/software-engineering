...
3. The `createClass()` method generates a unique `classId`, instantiates a new `SchoolClass` object: `classObj`  and invokes the `save(classId, classObj)` method within the `ClassRepository`.
