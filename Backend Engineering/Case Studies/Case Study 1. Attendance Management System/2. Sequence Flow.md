...
1. Upon submitting the form, the frontend sends a **`POST /createClass`** request to the **`AttendanceManagementSystem`**, including the input class details. 
2. The **`AttendanceManagementSystem`** extracts request parameters: **`className`** and **`teacherName`**, and invokes the **`createClass(className, teacherName)`** method within the **`ClassService`**. 
3. The **`createClass()`** method performs basic validations (e.g., check if **`className`** already exists), instantiates a new **`Class`** object: **`classObj`**  and invokes the **`save(classObj)`** method within the **`ClassRepository`**.
4. The **`save()`** method generates a unique **`classID`** and saves the **`classObj`** in the **in-memory** collection.
...
3. The **`markAttendance()`** method performs basic validations, such as ensuring that the **`Map<rollNumber, AttendanceStatus>`** for **`todayDate`** for the **target class** is not already present in the **`AttendanceRecord`**. 
4. If the validation succeeds, a new **`Map<todayDate, Map<rollNumber, AttendanceStatus>`** entry is added to the **`AttendanceRecord`** of the **target class** via the **`save(classId, Map<todayDate, Map<rollNumber, AttendanceStatus>)`** method of **`AttendanceRepository`**.
...
2. The **`AttendanceManagementSystem`** extracts request parameters: **`classId`**, **`year`** and **`month`**, and invokes the  **`generateAttendanceReport(classId,year, month)`** method within the **`ReportService`**.
3. The **`generateAttendanceReport()`** method invokes the **`getAttendanceRecord(classId)`** method within the **`AttendanceRepository`** to retrieve the **`AttendanceRecord`** for the specified class and performs basic validations such as ensuring that at least one record exist in the **`AttendanceRecord`** that fall within the given month. 
