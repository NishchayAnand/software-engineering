...
The **`Teacher`** begins the process by clicking the **"Mark Attendance"** button from the UI. The frontend sends a **`GET request`** to the **`AttendanceManagementSystem`** to fetch the list of existing classes.
...
Teacher selects "View Monthly Report"
    ↓
System prompts to:
    - Select a class
    - Select a month (e.g., August 2025)
    ↓
System filters in-memory attendance records for:
    - That class
    - All dates in the selected month
    ↓
System calculates for each student:
    - Total Present
    - Total Absent
    - Total Late
    - Total On Leave
    - Attendance %
    ↓
System displays a report like:

| Roll | Name      | P | A | L | O | %    |
|------|-----------|---|---|---|---|------|
| 1    | Alice     | 20| 2 | 1 | 0 | 91.3 |
| 2    | Bob       | 18| 3 | 2 | 0 | 86.9 |
