...
2. A `GET request` is sent to the `AttendanceManagementSystem` to pre-populate class dropdown menu with the list of existing classes.
...
1. Upon submitting the form, the frontend sends a `POST /addStudent` request to the `AttendanceManagementSystem`, including the selected `classId` and the student's details.
...
3. The `addStudent()` method validates whether a student with the same `enrollmentId` already exists in the target class. Upon successful validation, it instantiates a new `studentObject` and adds the the `{enrollmentId, studentObject}` combination to the student collection of the target class.
