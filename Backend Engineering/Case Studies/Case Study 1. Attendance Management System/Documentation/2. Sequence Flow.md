...
3. The `createClass()` method generates a unique `classId`, instantiates a new `ClassObject`  and invokes the `save(classId, classObject)` method within the `ClassRepository`.
