...
4. The `addStudent()` method validates if a student with the same `enrollmentId` already exists in the target class. Upon successful validation, it instantiates a new `studentObject` and adds the the `{enrollmentId, studentObject}` combination to the student collection of the target class.
