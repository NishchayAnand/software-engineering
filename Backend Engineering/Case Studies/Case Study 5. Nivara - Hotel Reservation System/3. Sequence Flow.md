...
The `ReservationService` can orchestrate the booking initiation and inventory locking phase. 

It coordinates with the `InventoryService` to place a **temporary hold** on the selected inventory, preventing double booking, and then calls the `PaymentService` to create a **payment gateway order**. 
...
The `PaymentService` orchestrates the payment processing and booking finalization phase. Why PaymentService and not ReservationService


In the **second phase**, after the user completes the payment, the **`PaymentService` becomes the orchestrator**. It verifies the payment with the external gateway, finalizes the inventory hold by coordinating with the `InventoryService`, and then calls the `ReservationService` to **confirm the reservation**. This transition ensures that reservations are only finalized after successful payment while keeping payment-related failures isolated from core booking state.
