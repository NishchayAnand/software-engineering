...
The **Reservation Service** is responsible for managing the **booking lifecycle**, including reservation creation, inventory locking coordination, and reservation state transitions. It owns the **reservation state machine** and ensures that bookings are created safely before payment is initiated.
...
Below, we discuss the key endpoints exposed by the Reservation Service.
#### 1. `POST /reservations`
...
public ReservationResponse createReservation(ReservationRequest request) {
	...
	Optional<Reservation> existing =
			reservationRepository.findByRequestId(request.getRequestId());
