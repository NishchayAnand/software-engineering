...
> 1250 RPS is ~1–5% of MakeMyTrip’s peak search traffic.

That might sound small — but architecturally?
...
Doing this 1250 times per second synchronously = disaster ❌
...
- 6–8 service instances
- Redis with ≥90% cache hit
...
This is a medium-load read-heavy use case. 50 → 250 RPS is a healthy, production-grade workload that demands good caching and clean service boundaries — but not extreme scaling tricks.
...
> View Details is ~5× lighter than Search, but heavier per request
...
- 2–3 stateless service instances
- One Redis cluster
- One read replica DB

Caching strategy (critical)

- Hotel metadata → cache TTL = hours
- Policies → cache TTL = hours
- Amenities → cache TTL = hours
- Availability → cache TTL = minutes
...
❌ Fetching everything synchronously
❌ No caching for availability
...
This is a classic low-QPS, high-risk use case. 1–5 RPS means you should design for safety, not scale.
...
- Even 1–2 instances can handle this load
...
At higher RPS, you’d avoid these — but here they’re safe.
...
Our use cases have very different characteristics:
...
| Payments           | <1      | 2â€“3    | External, idempotent             |
Mixing these in one service causes:
...
| Hotel Details | Metadata store   |
| Reservation   | Transactional DB |
| Payment       | Transactional DB |
...
- Reservation confirmed → Inventory updated
