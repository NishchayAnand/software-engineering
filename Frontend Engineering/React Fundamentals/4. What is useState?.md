...
This is the **functional form** of a state update. Instead of passing a new state directly, you pass a function that receives the **previous state** (`prev`).

<span style="color:red;">This pattern is used when the new state</span> **depends on the previous one**<span style="color:red;">, ensuring React gives you the most up-to-date value — even if multiple updates happen close together.</span>

> **NOTE:** If you mutated `prev` directly, React might not detect the change. You make a **shallow copy** of the previous state object using the spread operator: `const copy = { ...prev };`. This ensures **immutability** — you don’t directly modify the existing state object (React requires immutable updates).

| Line                       | Meaning                                                                 |
| -------------------------- | ----------------------------------------------------------------------- |
| `setSelections(prev => …)` | Use previous state safely                                               |
| `{ ...prev }`              | Create a shallow copy (immutability)                                    |
| `return copy`              | Return the new state object                                             |
