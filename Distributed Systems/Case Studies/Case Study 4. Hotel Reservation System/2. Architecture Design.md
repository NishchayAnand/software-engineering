...
Majority of the traffic comes from **login requests**. Since, most calls are short-lived database reads / writes and generate **moderate load**
...
- **Session & tokens:** Use **Redis** for short-lived data like session states, refresh tokens, and rate-limits.
