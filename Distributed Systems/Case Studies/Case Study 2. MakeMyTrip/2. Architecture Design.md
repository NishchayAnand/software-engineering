...
The next step is analyzing <span style="color:green;font-weight:bold;background:beige;">how traffic distributes across different user actions</span>. Let’s estimate the load for the three core use cases: 
...
In a hotel-reservation platform, natural seams are **User Service**, **Hotel Service**, **Reservation Service**, **Payment Service** and a **Notification Service**.

---

- **Catalog Service** (hotels, rooms, amenities, images metadata), 
- **User Service** (identity, profiles), 
- **Reservation Service** (ACID reservation workflow and idempotency),
- **Payment Service** (PCI-scoped gateway orchestration)
- **Notification Service** (email/SMS/push)

Additionally, Search Service, Inventory Service, Pricing Service (whys)???

Let's discuss each services in detail:

<span style="color:purple;font-weight:bold;">User Service</span>

<span style="color:purple;font-weight:bold;">Catalog Service</span>
