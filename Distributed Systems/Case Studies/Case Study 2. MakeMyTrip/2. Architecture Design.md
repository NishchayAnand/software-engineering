...
- Average Hotel Record Size = 5 KB  
    ...
    - Location: ~200 bytes (full address, city, state, country)  
...
- Average Room Record Size = 1 KB 
	- Room Type: ~100 bytes (e.g., "Deluxe", "Standard")  
    - Price: ~50 bytes (price value + currency)  
    - Availability: ~200 bytes (availability calendar)  
    - Other Attributes: ~650 bytes (bed type, occupancy, smoking policy, etc.)  
...
```
- (Assumption) Total Registered Users: 100 Million

- Average size per user profile: 1 KB
	- User ID: ~16 bytes (UUID or numeric ID)
	- Name: ~100 bytes (first and last name)
	- Email: ~50 bytes
	- Phone number: ~20 bytes 
	- Hashed password: ~256 bytes (depending on algorithm)
	- Basic preferences or flags: ~200 bytes
...
- Booking Record Size = 1 KB
	- user_id: ~8-16 bytes (UUID or numeric ID)
	- hotel_id: ~8-16 bytes
	- room_id: ~8-16 bytes
	- dates: ~16–32 bytes (check-in and check-out dates)
	- payment status: ~10–20 bytes (e.g., “Paid”, “Pending”)
...
> NOTE: Booking records an be moved to OLAP databases after 1 year.

<span style="color:green;font-weight:bold">Summary</span>

|**Entity**|**Assumptions**|**Estimated Size per Record**|**Total Records**|**Total Storage**|
|---|---|---|---|---|
|**Hotels**|1M hotels, each ~5 KB (name, location, description, amenities, image metadata)|~5 KB|1,000,000|~5 GB|
|**Hotel Images**|10 images per hotel, avg 500 KB each|500 KB|10,000,000|~5 TB|
|**Rooms**|10 rooms per hotel, each ~500 bytes (type, price, capacity)|~500 B|10,000,000|~5 GB|
|**Users**|10M users, each ~1 KB (id, name, email, preferences)|~1 KB|10,000,000|~10 GB|
|**Bookings**|Each booking record ~1 KB (user_id, hotel_id, room_id, dates, status)|~1 KB|100,000,000|~100 GB|
